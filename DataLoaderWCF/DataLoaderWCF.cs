...
{
    public class DataLoaderWCF : IDataLoaderWCF
    {
        public bool SubmitFile(Guid _customerId, POSFile _file)
        {
            ...
            try
            {
                ...
                if (null != customer)
                {
                    ValidateCustomerPath(customer);

                    var extension = Path.GetExtension(_file.FileName);
                    var path = Path.Combine(customer.UploadPath, _file.FileName);
        ...
        private static void ValidateCustomerPath(CustomerProperty customer)
        {
            if (!Directory.Exists(customer.UploadPath))
            {
                Directory.CreateDirectory(customer.UploadPath);
            }

            if (!Directory.Exists(customer.ArchivePath))
            {
                Directory.CreateDirectory(customer.ArchivePath);
            }
        }

        public bool LoadJournalFiles(Guid _customerId)
        {
            var customer = Jaxis.POS.CustomerData.CustomerProperty.All().FirstOrDefault(c => c.CustomerID == _customerId);
            LoadJournalFiles(customer);
            return true;
        }

        private void LoadJournalFiles(CustomerProperty customer)
        {
            if (!string.IsNullOrWhiteSpace(customer?.JournalParser))
            {
                var loader = new Micros.DataLoader.JournalLoader(customer.JournalParser, false, customer.ConnectionString);

                var files = Directory.GetFiles(customer.UploadPath, "*.jnl");
        ...
        public bool LoadDataFiles(Guid _customerId)
        {
            var customer = Jaxis.POS.CustomerData.CustomerProperty.All().FirstOrDefault(c => c.CustomerID == _customerId);
            ...
            if (null != customer)
            {
                ...
                var configItems = customer.DataFileConfig.Split('\n');
                var dataConfigItems = (from item in configItems where !string.IsNullOrWhiteSpace(item) select new DataFileConfig(item)).ToList( );

                var files = Directory.GetFiles(customer.UploadPath);

                foreach (var config in dataConfigItems)
                {
                    var fileList = files.Where(n => n.Replace( $"{customer.UploadPath}\\", "").StartsWith(config.FileName));
                    foreach (var file in fileList)
                    {
                        Log.Debug($"File: {file}");
                        ProcessFile(customer.ConnectionString, file, config.TableData, config.Delimiter, config.ExtraColumns);
                        filesToZip.Add(file);
                    }
                }
                LoadJournalFiles(customer);

                ArchiveFiles(customer);
            }
            return true;
        }

        private static void ArchiveFiles(CustomerProperty customer)
        {
            using (var zip = new ZipFile())
            {
                ...
                if (0 < files.Length)
                {
                    ...
                    zip.Save($"{customer.ArchivePath}{DateTime.Today.Year}.{DateTime.Today.Month}.{DateTime.Today.Day}.{DateTime.Now.Ticks}.zip");
        ...
        private static void ProcessFile(string connectionString, string file, string[] tableData, char delimiter, int extraColumns)
        {
            if (File.Exists(file))
            {
                Log.Debug($"Load File: {file}");

                var loader = new ExcelLoaders.CSVBulkLoad { ConnectionString = connectionString };
